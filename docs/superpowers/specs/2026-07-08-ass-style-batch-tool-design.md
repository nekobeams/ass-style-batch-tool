...
- 系統需安裝 **ffmpeg/ffprobe** 並存在於 PATH 中,用來偵測影片實際解析度(僅在 ASS 缺少有效 PlayResX/Y 時作為備援)
...
2. 若缺失或無效 → 嘗試用 ffprobe 偵測配對到的影片實際解析度作為參考
3. 皆不可得 → 不縮放(scale = 1),並在 log 記錄警告

**工具絕不改寫 `[Script Info]` 中的 `PlayResX`/`PlayResY` 標頭**——只更新目標 Style 那一行的數值,避免影響特效字幕的絕對座標。
...
4. 配對不到影片的字幕檔標記為「無對應影片」,自動退回使用該字幕檔自己的 PlayResX/Y
5. 若同一集數編號配對到多個候選字幕或影片(例如同集有兩種版本檔名),標記為「配對模糊」,該集不使用影片解析度備援,並在預覽與 log 中列出讓使用者自行確認
...
- ffprobe 未安裝/找不到 → 程式啟動時偵測並提示,仍可繼續執行(僅無法用影片解析度備援)
